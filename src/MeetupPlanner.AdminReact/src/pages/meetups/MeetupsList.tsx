      ...
      {viewMode === 'list' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {meetups.map((meetup) => (
            ...
            >
              <h3 className="text-lg font-semibold text-gray-900 mb-2">{meetup.title}</h3>
              <p className="text-sm text-gray-600">{new Date(meetup.startUtc).toLocaleDateString()}</p>
              <p className="text-sm text-gray-600">{meetup.location.name}</p>
              <p className="text-sm text-gray-600">RSVPs: {meetup.rsvp.rsvpYesCount} / {meetup.rsvp.totalSpots}</p>
            </Link>
          ))}
        </div>
      )}
